export async function createLightingPreview(canvas, presets){
 ...
 actor.add(makePlant('pothos',1));actor.rotation.y=-.08;refineMaterials(actor);scene.add(actor);
 ...
 return {select:id=>{const p=presets.find(x=>x.id===id);if(p)apply(p);},getState:()=>({time:current?.id,drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles}),dispose:()=>{observer.disconnect();renderer.dispose();}};
}
