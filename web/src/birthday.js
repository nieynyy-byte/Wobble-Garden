...
import {createBirthdayState,BIRTHDAY_VISIT_MS,birthdaySchedule} from './birthday-state.js';
export function createBirthday({scene,actor,storage,reducedMotion=false}){
 ...
 const state=createBirthdayState(storage),travelers=[];let clock=0,lastCamera=null,loading=null,view={active:false,visiting:false},error=false,loadedSlot=-1;const modelCache=new Map();
 ...
 const flowerColors=[pink,mat('#e9c86c'),mat('#bdade1')];
 function flowers(){const g=new T.Group();for(let i=0;i<3;i++){const x=(i-1)*.19,y=.25+(i===1?.16:0);mesh(new T.CylinderGeometry(.023,.023,y+.15,8),green,g,x,y/2-.05);const f=new T.Group();g.add(f);f.position.set(x,y,0);for(let j=0;j<6;j++){const a=j/6*Math.PI*2;const petal=mesh(new T.SphereGeometry(.105,12,8),flowerColors[i],f,Math.cos(a)*.13,Math.sin(a)*.13);petal.scale.set(1,1,.5);}mesh(new T.SphereGeometry(.1,12,8),cream,f,0,0,.035);for(const ex of [-.035,.035])mesh(new T.SphereGeometry(.018,8,6),dark,f,ex,.018,.13);}mesh(new T.ConeGeometry(.22,.4,16),mat('#d2c3e1'),g,0,-.08).rotation.z=Math.PI;return g;}
 ...
 const pairs=[['9944','9952'],['9945','9953'],['9947','9955'],['9950','9959'],['9960','9962'],['9970','9972'],['9983','9990'],['9986','9988'],['9941','9944']];
 async function load(slot){if(loading)return;loading=true;error=false;try{bounds();const pot=new T.Box3().setFromObject(actor.getObjectByName('Pot')).getSize(new T.Vector3()),loader=new GLTFLoader();const models=await Promise.all(pairs[slot%pairs.length].map(async id=>{if(!modelCache.has(id))modelCache.set(id,await loader.loadAsync(new URL('../public/assets/models/characters/'+(['9960','9972'].includes(id)?'logo-friends-v05':'logo-friends-v04')+'/img-'+id+'.glb',import.meta.url).href));return modelCache.get(id).scene.clone(true);}));for(const t of travelers){root.remove(t.root);t.gift.traverse(o=>o.geometry?.dispose());}travelers.length=0;models.forEach((model,index)=>{const t=createTraveler(model,{index,count:1,sizeFactor:1.64,potHeight:pot.y,potWidth:pot.x,reducedMotion,obstacle,anchorPosition:new T.Vector3(index?1.05:-1.05,2.9,1.9)});const gift=index?flowers():cake();gift.scale.setScalar(1.05);gift.position.set(0,-.20,.75);t.root.add(gift);t.gift=gift;t.presentation=null;root.add(t.root);travelers.push(t);});loadedSlot=slot;}catch{error=true;}finally{loading=false;}}
 ...
 return {root,update({owner,date=new Date(),time,dt,camera,visible,darkness=0,preview=false}){
  clock=time;lastCamera=camera;view=preview?{active:true,...birthdaySchedule(date),visiting:date.getHours()<21,age:Math.max(0,time*1000)%BIRTHDAY_VISIT_MS}:state.get(owner,date,visible);
  ...
  if(view.visiting&&!loading&&loadedSlot!==view.slot&&!error)void load(view.slot);
 ...
 },getPositions:()=>travelers.map(t=>t.root.position.clone()),getState:()=>({...view,loaded:travelers.length,loadedSlot,pair:pairs[(loadedSlot<0?0:loadedSlot)%pairs.length],error,presenting:travelers.map(t=>!!t.presentation),text3D:true}),gaze(){if(!root.visible||!view.visiting||!travelers.length)return null;const p=travelers[0].root.position;return {player:false,x:T.MathUtils.clamp(p.x*.045,-.06,.06),y:T.MathUtils.clamp((p.y-1.35)*.035,-.04,.06)};}};
}
