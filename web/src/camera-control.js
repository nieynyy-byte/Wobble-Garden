...
export function installCameraDrag(surface,{enabled,onChange,onFirstDrag,onGesture,panLimit=5,panSpeed=10,zoomMin=.72,zoomMax=1.3}){
 ...
 surface.addEventListener('pointerdown',e=>{if(!enabled()||(e.pointerType==='mouse'&&e.button!==0))return;if(!points.size){moved=false;suppressClick=false;}points.set(e.pointerId,{x:e.clientX,y:e.clientY});last=measure();},true);
 let pending=false;
 function applyMove(){if(!pending)return;pending=false;if(!points.size||!last)return;const next=measure();if(points.size>1)current.pan=clamp(current.pan-(next.x-last.x)/Math.max(280,surface.clientWidth)*panSpeed,-panLimit,panLimit);else current.x=clamp(current.x-(next.x-last.x)/Math.max(280,surface.clientWidth)*1.2,-.48,.48);if(next.gap&&last.gap)current.zoom=clamp(current.zoom*last.gap/next.gap,zoomMin,zoomMax);last=next;changed();}
 surface.addEventListener('pointermove',e=>{if(!enabled()||!points.has(e.pointerId))return;const old=points.get(e.pointerId);if(!moved&&Math.hypot(e.clientX-old.x,e.clientY-old.y)<6)return;points.set(e.pointerId,{x:e.clientX,y:e.clientY});if(!moved)onGesture?.();moved=true;suppressClick=true;surface.setPointerCapture(e.pointerId);e.preventDefault();if(!pending){pending=true;requestAnimationFrame(applyMove);}if(!hintShown){hintShown=true;onFirstDrag?.();}},{passive:false,capture:true});

 function end(e){if(e.type==='lostpointercapture'&&surface.hasPointerCapture(e.pointerId))return;if(!points.has(e.pointerId))return;applyMove();points.delete(e.pointerId);last=points.size?measure():null;released=performance.now();if(surface.hasPointerCapture(e.pointerId))surface.releasePointerCapture(e.pointerId);}
 for(const event of ['pointerup','pointercancel','lostpointercapture'])surface.addEventListener(event,end,true);
 ...
 const read=()=>({...current});read.isGesture=()=>moved&&(points.size>0||performance.now()-released<150);
