...
export function createDiceVisitor({scene,actor,camera,getSave,eligible,reducedMotion=false,register=()=>{}}){
 ...
 function update(dt,date=new Date()){
  ...
  if(age<16){const u=ease(age/16);root.position.copy(arrival.getPointAt(u));root.scale.setScalar(.18+.82*u);spinner.rotation.set(reducedMotion?0:(1-u)*Math.PI*2,reducedMotion?0:(1-u)*Math.PI*4,reducedMotion?0:(1-u)*Math.PI*2);}
   ...
   rollAge+=dt;const u=ease(rollAge/8);root.position.copy(rolling.getPointAt(u));spinner.rotation.set(reducedMotion?0:Math.PI*2*u,0,reducedMotion?0:Math.PI*4*u*(rolls%2?1:-1));
   if(rollAge>=8){rollAge=null;spinner.rotation.set(0,0,0);}
  }else{
   root.scale.setScalar(1);root.position.copy(anchor);if(!reducedMotion){root.position.y+=Math.sin(age*.65)*.075;root.position.x+=Math.sin(age*.3)*.05;spinner.rotation.set(Math.sin(age*.42)*.055,Math.sin(age*.32)*.12,Math.sin(age*.56)*.07);}
  }
 ...
 return {root,update,interact,getPosition:()=>root.position.clone(),getState:()=>({visible:root.visible,loaded,pending,error,age,rolls,rolling:rollAge!==null,position:root.position.toArray()})};
}
