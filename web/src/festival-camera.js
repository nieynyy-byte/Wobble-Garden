export function installFestivalCamera(surface,{enabled=()=>true}={}){
 ...
 surface.addEventListener('pointerdown',e=>{if(!enabled())return;points.set(e.pointerId,{x:e.clientX,y:e.clientY});surface.setPointerCapture(e.pointerId);last=measure();moved=false;},true);
 surface.addEventListener('pointermove',e=>{if(!enabled()||!points.has(e.pointerId))return;points.set(e.pointerId,{x:e.clientX,y:e.clientY});const next=measure();if(last){const dx=next.x-last.x;state.x=clamp(state.x-dx/surface.clientWidth*10,-5,6);if(next.gap&&last.gap)state.zoom=clamp(state.zoom*last.gap/next.gap,.65,1.5);moved||=Math.abs(dx)>2||!!next.gap;}last=next;e.preventDefault();}, {passive:false});
 ...
 surface.addEventListener('click',e=>{if(enabled()&&moved){e.preventDefault();e.stopImmediatePropagation();}},true);
 surface.addEventListener('wheel',e=>{if(!enabled())return;e.preventDefault();state.zoom=clamp(state.zoom*Math.exp(e.deltaY*.001),.65,1.5);released=performance.now();},{passive:false});
