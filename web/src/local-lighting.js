...
export function createLocalLighting(scene,renderer,room,{reducedMotion=false}={}){
 ...
 function update(date=new Date(),force=false){
  const time=localLightingTime(date),key=time.key+':'+date.getSeconds();
  ...
  current={...time,blend,moonPhase:lunar.phase,moonIllumination:lunar.illumination};
 }
 update();
 return {register,update,animate:seconds=>leaves.update(seconds,sun.intensity),getState:()=>({...current,sunPower:sun.intensity,ambient:hemi.intensity,night:U.nightLevel.value,moonPower:moon.intensity,leafShadows:leaves.getState()})};
}
