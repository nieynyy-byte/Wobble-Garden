...
function balloonGeometry(shape){
 ...
 function vertex(x,y){const i=y*nx+x;return {x:minX+x*step,y:minY+y*step,z:.36*Math.sqrt(pressure[i]/max),d:distances[i]};}
...
export function createBirthdayLettering(){
 const root=new T.Group(),letters=[],materials=[];root.name='3D bubble Happy Birthday';root.rotation.x=.08;root.rotation.y=-.09;
