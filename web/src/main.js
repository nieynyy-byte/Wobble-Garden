...
function swapPlant(id,days){if(!actor)return;crystal?.setDays(days??(id===save?.selectedPlant?save.wateringCount:0));const stage=growthStage(days??(id===save?.selectedPlant?save.wateringCount:0)),key=id+':'+stage;const original=actor.getObjectByName('SourcePothosSeedling');if(original)original.visible=false;for(const [k,p]of plants)p.visible=k===key;if(!plants.has(key)){const p=makePlant(id,stage);actor.add(p);plants.set(key,p);}plantRoot=plants.get(key);leaves=[];stems=[];plantRoot.traverse(o=>{if(o.name.startsWith('LeafPivot_'))leaves.push(base.has(o)?o:remember(o));});}
