...
function tapEye(side){if(!ready||blocked||!save?.selectedPlant||previewDay!==null||!$('welcome').hidden||$('details').open)return;guest?.tap().then(started=>{if(started)easterAt=-100;});tap={side,at:now};sequence=eyeSequence(sequence,side,performance.now());if(sequence.triggered&&now-easterAt>5&&!guest?.getState().active){easterAt=now;easterVariant=Math.random()<.5?0:1;}}
...
async function boot(){
 ...
 actor.rotation.y=-.08;refineMaterials(actor);guest=createGuest9978({scene,actor,storage,reducedMotion:reduced,eligible:()=>ready&&!blocked&&!!save?.selectedPlant&&previewDay===null&&$('welcome').hidden&&!$('details').open});localLighting=createLocalLighting(scene,renderer,env);localLighting.register(actor);const sourcePlant=actor.getObjectByName('PothosSeedling');sourcePlant.name='SourcePothosSeedling';sourcePlant.visible=false;
...
function frame(ms){requestAnimationFrame(frame);if(document.hidden){lastTime=ms;return;}if(ms-lastFrame<1000/30)return;const dt=Math.min(.08,(ms-lastTime)/1000||0);lastTime=ms;lastFrame=ms;now+=dt;
 ...
 if(!freeze){
  const idle=reduced?0:Math.sin(now*.37)*.014;pupil.position.x+=idle;if(guest?.root.visible){pupil.position.x+=.025;pupil.position.y+=.03;pupil.position.z+=side==='Left'?.013:-.002;}
