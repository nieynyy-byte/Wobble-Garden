...
syncOwner();
$('birthday-form').onsubmit=e=>{e.preventDefault();const value=$('birthday-month').value+'-'+String($('birthday-day').value).padStart(2,'0');$('birthday-status').textContent=ownerAccess.setBirthday(value)?'Birthday saved. A little surprise awaits.':'Please check the day and month, and allow browser storage.';};
...
function frame(ms){requestAnimationFrame(frame);if(document.hidden){lastTime=ms;return;}if(ms-lastFrame<1000/30)return;const dt=Math.min(.08,(ms-lastTime)/1000||0);lastTime=ms;lastFrame=ms-(ms-lastFrame)%(1000/30);now+=dt;
 ...
 birthday?.update({owner:ownerAccess.getState(),date:new Date(),time:now,dt,camera,visible:!blocked&&$('welcome').hidden&&(previewDay===null||birthdayReview),darkness:nightAmount(visualDate),preview:birthdayReview});
 const birthdayParty=birthday?.getState().active&&(previewDay??save?.wateringCount)>=45;festivalActive=festivalReview||!!birthdayParty;
