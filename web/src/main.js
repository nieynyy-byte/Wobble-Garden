...
import {SAVE_KEY,dayKey,loadStorage,persist,choosePlant,isLocked,water,eyeSequence} from './state.js';
import {PLANTS,plantById} from './plants.js';
import {makeRoom,makePlant} from './scenery.js';
import {refineMaterials} from './materials.js';
import {roomReflections} from './daylight.js';
import {installCameraDrag} from './camera-control.js';
import {growthForDays,growthStage,PREVIEW_DAYS} from './growth.js';
...
function storageStatus(ok){if(!ok)$('save-note').textContent='This browser cannot save your garden. Progress may be lost after closing.';}
...
function updateCare(){if(!save)return;if(previewDay!==null){$('garden-name').textContent='Pothos · growth preview';return;}$('water').querySelector('span').textContent=save.lastWateringDate===dayKey()?'A little more water?':'Water your plant';$('garden-name').textContent=save.plantName||plantById(save.selectedPlant).name;}
function persistSave(){const ok=persist(storage,save);storageStatus(ok);return ok;}
function swapPlant(id,days){if(!actor)return;const stage=id==='pothos'?growthStage(days??(id===save?.selectedPlant?save.wateringCount:0)):1,key=id+':'+stage;const original=actor.getObjectByName('SourcePothosSeedling');if(original)original.visible=false;for(const [k,p]of plants)p.visible=k===key;if(!plants.has(key)){const p=makePlant(id,stage);actor.add(p);plants.set(key,p);}plantRoot=plants.get(key);leaves=[];stems=[];plantRoot.traverse(o=>{if(o.name.startsWith('LeafPivot_'))leaves.push(base.has(o)?o:remember(o));});}
function previewGrowth(day=0){if(!ready||blocked)return;previewDay=Math.max(0,Math.min(45,Math.floor(Number(day)||0)));$('details').close();$('welcome').hidden=true;wrap.inert=false;document.body.classList.add('previewing');$('growth-preview').hidden=false;$('water').disabled=true;swapPlant('pothos',previewDay);const g=growthForDays(previewDay);$('preview-title').textContent=g.name;$('preview-day-label').textContent=previewDay+' watering days';$('preview-description').textContent=previewDay===45?'Fully grown. Day 45 keeps this mature plant.':g.description;$('preview-range').value=previewDay;document.querySelectorAll('[data-preview-day]').forEach(b=>b.setAttribute('aria-pressed',String(Number(b.dataset.previewDay)===previewDay)));updateCare();}
...
$('preview-range').oninput=e=>previewGrowth(e.target.value);$('exit-preview').onclick=closePreview;$('open-growth-preview').onclick=()=>previewGrowth(save?.selectedPlant==='pothos'?save.wateringCount:0);
...
async function start(e){e?.preventDefault();if(blocked||!ready)return;const commit=()=>{if(previewDay!==null)return;const latest=loadStorage(storage);if(latest.status==='future'){blocked=true;message('Open the latest version to continue.');return;}if(['loaded','migrated'].includes(latest.status))save=latest.save;const result=choosePlant(save,selected,$('plant-name').value);if(!result.accepted){if(result.reason==='locked'){home();message('Your plant stays with you for 45 watering days.');}else $('plant-name').reportValidity();return;}save=result.save;persistSave();enterGarden();message('Hello, '+save.plantName+'. Make yourself at home.');};if(navigator.locks)await navigator.locks.request('wobble-water',commit);else commit();}
$('choose-form').addEventListener('submit',start);$('home').onclick=home;$('return').onclick=enterGarden;
$('info').onclick=()=>{if(!save?.selectedPlant)return home();const p=plantById(save.selectedPlant);$('detail-name').textContent=save.plantName||p.name;$('detail-species').textContent=p.name+' · '+p.latin;$('detail-copy').textContent=p.detail;$('detail-progress').textContent=save.wateringCount+' / 45 watering days together'+(save.selectedPlant==='pothos'?' · '+growthForDays(save.wateringCount).name:'');$('details').showModal();};$('close-details').onclick=()=>$('details').close();
async function doWater(){if(previewDay!==null)return;if(!ready||blocked||!save?.selectedPlant)return;
 const commit=()=>{if(previewDay!==null)return;const latest=loadStorage(storage);if(latest.status==='future'){blocked=true;message('Please open the latest version to continue.');return;}if(['loaded','migrated'].includes(latest.status))save=latest.save;
 const result=water(save);save=result.save;persistSave();waterAt=now;waterRepeat=result.repeats;swapPlant(save.selectedPlant);updateCare();
 message(result.advanced?(save.wateringCount===45?'Forty-five little visits. Look how you have grown.':'A little drink. Thank you.'):waterRepeat===2?'Oh… another little drink?':waterRepeat===3?'I think my eyes are floating.':'A plant, or a tiny pond?');};
...
window.addEventListener('storage',e=>{if(e.key!==SAVE_KEY)return;const r=loadStorage(storage);if(r.status==='future'){blocked=true;$('water').disabled=true;message('Please open the latest version of this garden.');}else if(['loaded','migrated'].includes(r.status)){save=r.save;updateCare();if(previewDay!==null)return;if($('welcome').hidden)swapPlant(save.selectedPlant);else home();}});
...
async function boot(){
 ...
 scene=new THREE.Scene();scene.background=new THREE.Color('#c3cbb9');const reflections=roomReflections(renderer);scene.environment=reflections.texture;scene.environmentIntensity=.38;
 camera=new THREE.PerspectiveCamera(34,1,.1,40);camera.position.set(.25,3.4,8.8);camera.lookAt(0,1.8,0);
 scene.add(new THREE.HemisphereLight('#e6eef1','#a29a77',.45));const sun=new THREE.DirectionalLight('#fff0ce',2.9);sun.position.set(-3,7,-4);sun.castShadow=true;sun.shadow.mapSize.set(1024,1024);sun.shadow.camera.left=-3;sun.shadow.camera.right=3;sun.shadow.camera.top=4;sun.shadow.camera.bottom=-3;sun.shadow.normalBias=.012;sun.shadow.bias=-.0002;sun.shadow.radius=5;sun.shadow.blurSamples=8;scene.add(sun);const bounce=new THREE.DirectionalLight('#ffeddb',.55);bounce.position.set(1,3,6);scene.add(bounce);
 const loader=new GLTFLoader();const g=await loader.loadAsync('./public/assets/models/wobble-pothos-v03.glb');
 actor=g.scene;env=makeRoom();scene.add(actor,env);
 ...
 actor.rotation.y=-.08;refineMaterials(actor);const sourcePlant=actor.getObjectByName('PothosSeedling');sourcePlant.name='SourcePothosSeedling';sourcePlant.visible=false;
