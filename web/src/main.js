...
async function boot(){
 ...
 if(festivalReview){camera.far=90;camera.updateProjectionMatrix();festival=await (await import('./festival.js')).createFestival({scene,renderer,reducedMotion:reduced,tier:ownerAccess.getState().owner?'owner':'free'});festivalCamera=installFestivalCamera(wrap);reviewHour=21;}
...
installCameraDrag(wrap,{enabled:()=>ready&&!blocked&&!festivalReview&&$('welcome').hidden&&!$('details').open,onChange:p=>{pan=p;},onFirstDrag:()=>message('A little look around.')});
